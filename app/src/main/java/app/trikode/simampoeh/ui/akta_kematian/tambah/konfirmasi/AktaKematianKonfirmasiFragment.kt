package app.trikode.simampoeh.ui.akta_kematian.tambah.konfirmasi
...
import app.trikode.simampoeh.databinding.FragmentAktaKematianKonfirmasiBinding
...
class AktaKematianKonfirmasiFragment : Fragment(), View.OnClickListener {

    private var _binding: FragmentAktaKematianKonfirmasiBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAktaKematianKonfirmasiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onClick(v: View) {
        when(v.id) {
            binding.btnKirim.id -> {
                showLoading(true)
                val jsonString = Gson().toJson(tambahAktaKematianViewModel.dataPengajuan)
                Log.d("JSON_STRING", jsonString)
                kirimViewModel.submitLayanan("tambah_aktamati", jsonString)
            }
            binding.btnBack.id -> {
                if (sendResult == 0){
    ...
    private fun joinAllData() {
        tambahAktaKematianViewModel.dataPengajuan?.kirim = formTujuanKirimViewModel.getTujuanKirim().kirim
        tambahAktaKematianViewModel.dataPengajuan?.idProvinsi = formTujuanKirimViewModel.getTujuanKirim().idProvinsi
        tambahAktaKematianViewModel.dataPengajuan?.idKabupaten = formTujuanKirimViewModel.getTujuanKirim().idKabupaten
        tambahAktaKematianViewModel.dataPengajuan?.idKecamatan = formTujuanKirimViewModel.getTujuanKirim().idKecamatan
        tambahAktaKematianViewModel.dataPengajuan?.idKelurahan = formTujuanKirimViewModel.getTujuanKirim().idKelurahan
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance() = AktaKematianKonfirmasiFragment()
    }
}