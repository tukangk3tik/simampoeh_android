package app.trikode.simampoeh.ui.kia.tambah.konfirmasi
...
import app.trikode.simampoeh.databinding.FragmentKiaKonfirmasiBinding
...
class KiaKonfirmasiFragment : Fragment(), View.OnClickListener {

    private var _binding: FragmentKiaKonfirmasiBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentKiaKonfirmasiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onClick(v: View) {
        when(v.id) {
            binding.btnKirim.id -> {
                showLoading(true)
                val jsonString = Gson().toJson(tambahKiaViewModel.dataPengajuan)
                Log.d("JSON_STRING", jsonString)
                kirimViewModel.submitLayanan("tambah_identitasanak", jsonString)
            }
            binding.btnBack.id -> {
                if (sendResult == 0){
    ...
    private fun joinAllData() {
        tambahKiaViewModel.dataPengajuan?.kirim = formTujuanKirimViewModel.getTujuanKirim().kirim
        tambahKiaViewModel.dataPengajuan?.idProvinsi = formTujuanKirimViewModel.getTujuanKirim().idProvinsi
        tambahKiaViewModel.dataPengajuan?.idKabupaten = formTujuanKirimViewModel.getTujuanKirim().idKabupaten
        tambahKiaViewModel.dataPengajuan?.idKecamatan = formTujuanKirimViewModel.getTujuanKirim().idKecamatan
        tambahKiaViewModel.dataPengajuan?.idKelurahan = formTujuanKirimViewModel.getTujuanKirim().idKelurahan
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance() = KiaKonfirmasiFragment()
    }
}