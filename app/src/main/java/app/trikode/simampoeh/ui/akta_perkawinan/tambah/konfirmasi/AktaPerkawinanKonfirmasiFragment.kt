package app.trikode.simampoeh.ui.akta_perkawinan.tambah.konfirmasi
...
import app.trikode.simampoeh.databinding.FragmentAktaPerkawinanKonfirmasiBinding
...
class AktaPerkawinanKonfirmasiFragment : Fragment(), View.OnClickListener {
    
    private var _binding: FragmentAktaPerkawinanKonfirmasiBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAktaPerkawinanKonfirmasiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        joinAllData()
        //d("DATA_KIRIM", tambahAktaPerkawinanViewModel.dataPengajuan.toString())
    ...
    override fun onClick(v: View) {
        when(v.id) {
            binding.btnKirim.id -> {
                showLoading(true)
                val jsonString = Gson().toJson(tambahAktaPerkawinanViewModel.dataPengajuan)
                d("DATA_KIRIM", tambahAktaPerkawinanViewModel.dataPengajuan.toString())
                kirimViewModel.submitLayanan("tambah_aktakawin", jsonString)
            }
            binding.btnBack.id -> {
                if (sendResult == 0){
    ...
    private fun joinAllData() {
        tambahAktaPerkawinanViewModel.dataPengajuan?.kirim = formTujuanKirimViewModel.getTujuanKirim().kirim
        tambahAktaPerkawinanViewModel.dataPengajuan?.idProvinsi = formTujuanKirimViewModel.getTujuanKirim().idProvinsi
        tambahAktaPerkawinanViewModel.dataPengajuan?.idKabupaten = formTujuanKirimViewModel.getTujuanKirim().idKabupaten
        tambahAktaPerkawinanViewModel.dataPengajuan?.idKecamatan = formTujuanKirimViewModel.getTujuanKirim().idKecamatan
        tambahAktaPerkawinanViewModel.dataPengajuan?.idKelurahan = formTujuanKirimViewModel.getTujuanKirim().idKelurahan
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance() = AktaPerkawinanKonfirmasiFragment()
    }
}