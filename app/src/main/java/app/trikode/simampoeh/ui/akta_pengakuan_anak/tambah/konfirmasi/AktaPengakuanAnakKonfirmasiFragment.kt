package app.trikode.simampoeh.ui.akta_pengakuan_anak.tambah.konfirmasi
...
import app.trikode.simampoeh.databinding.FragmentAktaPengakuanAnakKonfirmasiBinding
...
class AktaPengakuanAnakKonfirmasiFragment : Fragment(), View.OnClickListener {

    private var _binding: FragmentAktaPengakuanAnakKonfirmasiBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAktaPengakuanAnakKonfirmasiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onClick(v: View) {
        when(v.id) {
            binding.btnKirim.id -> {
                showLoading(true)
                val jsonString = Gson().toJson(tambahAktaPengakuanAnakViewModel.dataPengajuan)
                //Log.d("JSON_STRING", jsonString)
                kirimViewModel.submitLayanan("tambah_aktaaku", jsonString)
            }
            binding.btnBack.id -> {
                if (sendResult == 0){
    ...
    private fun joinAllData() {
        tambahAktaPengakuanAnakViewModel.dataPengajuan?.kirim = formTujuanKirimViewModel.getTujuanKirim().kirim
        tambahAktaPengakuanAnakViewModel.dataPengajuan?.idProvinsi = formTujuanKirimViewModel.getTujuanKirim().idProvinsi
        tambahAktaPengakuanAnakViewModel.dataPengajuan?.idKabupaten = formTujuanKirimViewModel.getTujuanKirim().idKabupaten
        tambahAktaPengakuanAnakViewModel.dataPengajuan?.idKecamatan = formTujuanKirimViewModel.getTujuanKirim().idKecamatan
        tambahAktaPengakuanAnakViewModel.dataPengajuan?.idKelurahan = formTujuanKirimViewModel.getTujuanKirim().idKelurahan
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance() = AktaPengakuanAnakKonfirmasiFragment()
    }
}