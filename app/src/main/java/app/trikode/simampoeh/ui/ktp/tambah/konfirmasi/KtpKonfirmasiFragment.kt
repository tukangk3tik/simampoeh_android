package app.trikode.simampoeh.ui.ktp.tambah.konfirmasi
...
import app.trikode.simampoeh.databinding.FragmentKtpKonfirmasiBinding
...
class KtpKonfirmasiFragment : Fragment(), View.OnClickListener {
    
    private var _binding: FragmentKtpKonfirmasiBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentKtpKonfirmasiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onClick(v: View) {
        when(v.id) {
            binding.btnKirim.id -> {
                showLoading(true)
                val jsonString = Gson().toJson(tambahKtpViewModel.dataPengajuan)
                kirimViewModel.submitLayanan("tambah_ktp", jsonString)
            }
            binding.btnBack.id -> {
                if (sendResult == 0){
    ...
    private fun joinAllData() {
        tambahKtpViewModel.dataPengajuan?.kirim = formTujuanKirimViewModel.getTujuanKirim().kirim
        tambahKtpViewModel.dataPengajuan?.idProvinsi = formTujuanKirimViewModel.getTujuanKirim().idProvinsi
        tambahKtpViewModel.dataPengajuan?.idKabupaten = formTujuanKirimViewModel.getTujuanKirim().idKabupaten
        tambahKtpViewModel.dataPengajuan?.idKecamatan = formTujuanKirimViewModel.getTujuanKirim().idKecamatan
        tambahKtpViewModel.dataPengajuan?.idKelurahan = formTujuanKirimViewModel.getTujuanKirim().idKelurahan
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance() = KtpKonfirmasiFragment()
    }
}