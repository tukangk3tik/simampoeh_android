package app.trikode.simampoeh.ui.akta_pengangkatan_anak.tambah.konfirmasi
...
import app.trikode.simampoeh.databinding.FragmentAktaAngkatAnakKonfirmasiBinding
import app.trikode.simampoeh.ui.akta_pengangkatan_anak.tambah.TambahAktaPengangkatanAnakViewModel
...
class AktaAngkatAnakKonfirmasiFragment : Fragment(), View.OnClickListener {

    private var _binding: FragmentAktaAngkatAnakKonfirmasiBinding? = null
    private val binding get() = _binding!!

    private val tambahAktaPengangkatanAnakViewModel: TambahAktaPengangkatanAnakViewModel by activityViewModels()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentAktaAngkatAnakKonfirmasiBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onClick(v: View) {
        when(v.id) {
            binding.btnKirim.id -> {
                showLoading(true)
                val jsonString = Gson().toJson(tambahAktaPengangkatanAnakViewModel.dataPengajuan)
                //Log.d("JSON_STRING", jsonString)
                kirimViewModel.submitLayanan("tambah_angkatanak", jsonString)
            }
            binding.btnBack.id -> {
                if (sendResult == 0){
    ...
    private fun joinAllData() {
        tambahAktaPengangkatanAnakViewModel.dataPengajuan?.kirim = formTujuanKirimViewModel.getTujuanKirim().kirim
        tambahAktaPengangkatanAnakViewModel.dataPengajuan?.idProvinsi = formTujuanKirimViewModel.getTujuanKirim().idProvinsi
        tambahAktaPengangkatanAnakViewModel.dataPengajuan?.idKabupaten = formTujuanKirimViewModel.getTujuanKirim().idKabupaten
        tambahAktaPengangkatanAnakViewModel.dataPengajuan?.idKecamatan = formTujuanKirimViewModel.getTujuanKirim().idKecamatan
        tambahAktaPengangkatanAnakViewModel.dataPengajuan?.idKelurahan = formTujuanKirimViewModel.getTujuanKirim().idKelurahan
    }
    ...
    companion object {
        @JvmStatic
        fun newInstance() = AktaAngkatAnakKonfirmasiFragment()
    }
}